...
from extended_sphere_space import ExtendedNSphereSpace
...
def parse_args():
    ...
    parser.add_argument(
        "--space-type", type=str, default="box", choices=("box", "sphere", "unbounded", "extended_sphere")
    )
    parser.add_argument(
        "--m-p",
        type=int,
    ...
    # AnInfoNCE changes
    parser.add_argument("--gt-lambda-1", default=5, type=float,
                        help="Ground truth Lambda values for the first n/2 dimensions.")
    parser.add_argument("--gt-lambda-2", default=25, type=float,
                        help="Ground truth Lambda values for the first n/2 dimensions.")
    parser.add_argument("--linearly-spaced-lambda", action="store_true")
    parser.add_argument("--lr-lambda", default=1e-3, type=float)
...
def main():
    ...
    elif args.space_type == "extended_sphere":
        space=ExtendedNSphereSpace(args.n)
    else:
    ...
    if args.p:
        ...
        lambda_init = torch.randn(args.n)*0.1
        lambda_train = torch.Tensor(lambda_init).to(device)
        lambda_train = lambda_train.requires_grad_()
        lambda_activation = torch.nn.functional.softplus
        print('lambda init: ', lambda_init)
        loss = losses.AnInfoNCELoss(batch_size=args.batch_size, lambda_train=lambda_train, lambda_activation=lambda_activation)
    else:
    ...
    if args.c_p:
        ...
        if args.space_type == "extended_sphere":  
            if args.linearly_spaced_lambda:
                gt_lambda = torch.Tensor(np.linspace(args.gt_lambda_1, args.gt_lambda_2, args.n)).to(device)
            else:
                gt_lambda = args.gt_lambda_1 * torch.ones(args.n, requires_grad=False, device=device)
                gt_lambda[:int(args.n/2)] = args.gt_lambda_2 
                
            print('ground truth Lambda: ', gt_lambda)
            sample_conditional = lambda space, z, gt_lambda, size, device=device: space.rejected_projected_normal(z, gt_lambda, size, threshold=args.threshold, device=device)
        else:
            sample_conditional = (
    ...
    if args.space_type == "extended_sphere":  
        def sample_marginal_and_conditional(size, gt_lambda=gt_lambda, device=device):
            z = latent_space.sample_marginal(size=size, device=device)
            z_tilde = latent_space.sample_conditional(z, size=size, gt_lambda=gt_lambda, device=device)
    ...
    for test in test_list:
        ...
        if args.space_type == "extended_sphere":
            optimizer = torch.optim.Adam([
                                      {'params': f.parameters()},
                                      {'params': [lambda_train], 'lr': args.lr_lambda},
        ], lr=args.lr)
        else:
        ...
        while (
            ...
            if global_step % args.n_log_steps == 1 or global_step == args.n_steps:
                ...
                if args.sphere_norm:
                    print(f"r: {f[-1].r}")
                if args.space_type == "extended_sphere":
                    l_values = list(lambda_activation(lambda_train).cpu().detach().numpy())
                    l_values = [f'{v:.1f}' for v in l_values]
                    l_values = '[' + ', '.join(l_values) + ']'
                    print(f"learned lambda values: {l_values} \t",)
